    ...
    <section class="browse-range py-4 text-center">
      ...
      <div class="categories d-flex justify-content-around">
        <div class="category  p-3 rounded-3 text-center" style="width: 30%;">
          <img src="../assets/home-dining.png" alt="Dining" class="img-fluid rounded-3 mb-3"
            style="height: 480px; object-fit: cover;">
          <h4>Dining</h4>
        </div>
        <div class="category bg-light p-3 rounded-3 text-center" style="width: 30%;">
          <img src="../assets/home-living.png" alt="Living" class="img-fluid rounded-3 mb-3"
            style="height: 480px; object-fit: cover;">
          <h4>Living</h4>
        </div>
        <div class="category bg-light p-3 rounded-3 text-center" style="width: 30%;">
          <img src="../assets/home-bedroom.png" alt="Bedroom" class="img-fluid rounded-3 mb-3"
            style="height: 480px; object-fit: cover;">
          <h4>Bedroom</h4>
        </div>
      </div>
      ...
      <div class="product-list d-flex flex-wrap justify-content-around">
        ...
        <div v-if="products.length > 0" class="row p-4">
          <div v-for="product in products" :key="product._id" class="col-md-3 col-6 mb-4">
            <div class="card">
              <img :src="product.image_path" class="card-img-top" style="height: 300px; object-fit: cover; cursor: pointer;" @click="goToProduct(product._id)"/>
              <div class="card-body">
                <h6 class="fw-bold" @click.stop="goToProduct(product._id)" style="cursor: pointer;">
                  {{ product.name }}
                </h6>
                <p class="card-text">{{ product.category }}</p>
                <div class="d-flex justify-content-between align-items-center">
                  <h6 class="fw-bolder">${{ product.price }}</h6>
                  <button class="btn border-black" @click.stop="addToCart(product)" style="cursor: pointer;">
                    Add to Cart
                  </button>
                </div>
              </div>
          ...
          <nav aria-label="Page navigation example">
            <ul class="pagination">
              ...
              <li class="page-item text-black" @click="handleNavigate" :class="{ disabled: currentPage == pageNumb }">
                <span class="page-link text-black bg-tint"><span aria-hidden="true">&raquo;</span></span>
              </li>
            </ul>
          </nav>
        </div>
    ...
    <section class="inspiration d-flex justify-content-center py-4 px-5 ">
      <div class="inspiration-content d-flex w-80">
        <div class="inspiration-text flex-1 pe-4">
          ...
          <p class="mb-3 text-muted">Our designer already made a lot of beautiful prototype of rooms that inspire you
          </p>
          <button>Explore More</button>
        </div>
        <div class="slideshow flex-2 position-relative">
          <div class="slide position-absolute w-80" v-for="(slide, index) in slides" :key="index"
            :class="{ active: currentSlide === index }">
            <img :src="slide.image" :alt="slide.title" class="img-fluid rounded-3 mb-3"
          ...
          <button class="arrow next position-absolute top-50 end-0 translate-middle-y bg-dark text-white border-0 p-2"
...
export default {
  ...
  methods: {
    ...
    goToProduct(productId) {
      this.$router.push({ name: "ProductDetailView", params: { id: productId } });
    },
  },
  mounted() {
    this.loadProducts();
